<template>
    <img class="icon-prev" src="/icon-prev.svg" alt="icon-prev" @click="goToPrev">
    <div class="list">
        <Carousel :value="visibleProducts" :numVisible="numVisible" :numScroll="1">
    ...
    <img class="icon-next" src="/icon-next.svg" alt="icon-next" @click="goToNext">
</template>
...
const visibleProducts = computed(() => {
    return filteredProducts.value.slice(currentIndex.value, currentIndex.value + numVisible.value);
});
...
const goToNext = () => {
    if (currentIndex.value < filteredProducts.value.length - numVisible.value) {
        currentIndex.value++;
    }
};
